...
from crust_lite.external_geology import load_known_fault_features
...
def write_summary(
    config: AppConfig,
    paths: ProjectPaths,
    known_structure_mode: str | None = None,
) -> Path:
    ...
    known, known_provenance = load_known_fault_features(paths, mode=known_structure_mode)
    ...
    lines = [
        ...
        f"- known structure status: {known_provenance.get('status', 'unknown')}",
        "- known structures are posthoc correlation/viewer overlays only and never analysis input.",
        ...
        _markdown_table(
            top_faults,
            ["segment_id", "n_events", "fault_score", "confidence"],
        ),
        "",
        ...
        f"- database path: `{db_meta.get('path', 'not_available')}`",
        f"- database role: `{db_meta.get('role', 'not_available')}`",
        f"- source policy: `{db_meta.get('source_policy', 'not_available')}`",
        f"- database verified: `{db_meta.get('verified', False)}`",
        f"- verified tables: {', '.join(db_meta.get('verification', {}).get('tables', [])) or 'not_available'}",
...
def export_outputs(
    ...
    maps = (
        write_static_maps(config, paths, known_structure_mode=known_structure_mode)
        if config.outputs.write_maps
        else {"map_files": []}
    )
    report = (
        write_summary(config, paths, known_structure_mode=known_structure_mode)
